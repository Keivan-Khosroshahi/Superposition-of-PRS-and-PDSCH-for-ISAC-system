...
for mont=1:1
%%%%%%%%%%%%%%% comb size 6 12 %%%%%%%%%%%%%%%%%% doppler added correctly
%%%%%%%%%%%%%%% two targets
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if any(~cellfun(@isempty, {prsInd}))
            ...
            for i=(1+prs.SymbolStart):prs.NumPRSSymbols+1
                ...
                ifftprs_tot = ifftprs_tot + ifftvector;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            rl_prs_dmrs(counter,:) = abs(ifft(dmrsifftvec/max(dmrsifftvec) + ifftprs_tot/max(ifftprs_tot),ifftsizedmrs*dmrsifftscaler));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Error_ratio(mont,:) = errorratio;
    BER(mont) = sum(errorratio)/(carrier.SlotsPerFrame);
    BR(mont) = 1e-6*maxThroughput(snrIdx)/(nFrames*10e-3) - sum(errorratio)*1e-6*maxThroughput(snrIdx)/(nFrames*10e-3)/(carrier.SlotsPerFrame);
...
% 
% % Save the cropped plot as a PDF file
% exportgraphics(gcf, 'range_prs4_dmrs_alpha1.pdf', 'ContentType', 'vector');
...
% 
% % Save the cropped plot as a PDF file
% exportgraphics(gcf, 'range_prs12_dmrs_alpha6.pdf', 'ContentType', 'vector');


% 
% 
figure;
plot(rangeresolution*(0:(size(r_l_dmrs,2)-1)),mean(rl_prs_dmrs),'DisplayName','Estimation');
ylabel('Magnitude','FontSize',14)
xlabel('Distance (m)','FontSize',14)
